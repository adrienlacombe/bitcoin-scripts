# Direct base-16 Winternitz

`FastWinternitz<N, H, P>` signs the message’s actual high/low nibbles and a
separate checksum. Recovery methods leave authenticated nibbles for a Script
consumer; terminal methods consume them. See the [overview](../README.md)
for the construction choice and the [shared types](../shared/README.md) for
hash and initial-secret representations.
...
These base-16 profiles compare serialized script plus witness cost for the
same message size and recovery or terminal contract. The hybrid retains 20-byte HASH160 public commitments. Both contribute
...
separation. Run `python3 src/signatures/winternitz/base16/tests/vectors.py` to reproduce the
independent Python stdlib endpoint/signature digests for all three hash profiles; Rust
compares those fixed vectors in `tests/hash_choice.rs`.


## Source and tests

- [mod.rs](mod.rs): typed keys, signing, checksum, and verifier profiles.
- [strided.rs](strided.rs): quotient/parity witness and chain tables.
- [tests/](tests/): signature, exact/clamped/numeric chain, strided, hash-choice,
  and initial-secret tests; [vectors.py](tests/vectors.py) independently
  reproduces all hash profiles.

Run `cargo test --locked --lib signatures::winternitz::base16` and
`python3 src/signatures/winternitz/base16/tests/vectors.py`.
